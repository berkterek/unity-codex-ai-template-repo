...
- For VContainer errors: check registration order, lifetime mismatches, missing
  `.As<Interface>()` calls.
- For NullReferenceException: identify which object is null and why (not injected?
  destroyed? never assigned?).
- For ECS bugs: check system update order, entity archetype, ECB playback timing.
- For UniTask bugs: check cancellation token state, .Forget() vs awaited, exception
  swallowing.
...
- If a test was missing that would have caught this, note it (but don't write it
  unless asked).
...
- Minimal fix applied.
- Verification steps.
- Missing test that would have caught this (if any).
