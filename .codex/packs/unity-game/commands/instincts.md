...
Instincts are atomic learned behaviors captured from session observations and distilled at session end. Each instinct has: a trigger, an action, a confidence score (0.3–0.9), an evidence count, and a scope (project or global).

Storage:
- Project instincts: `.codex/state/instincts/project/<project-hash>/*.json`
- Global instincts:  `.codex/state/instincts/global/*.json`
- Raw observations:  `.codex/state/instincts/observations.jsonl`
...
2. List files under `.codex/state/instincts/project/<hash>/` and `.codex/state/instincts/global/`.
...
4. Output the draft to stdout; do NOT write it. Tell the user:
   > Draft skill at `.codex/packs/unity-game/skills/core/learned-<domain>-patterns/SKILL.md` — review before installing.
...
1. Read `.codex/state/instincts/project/<project-hash>/<instinct-id>.json`.
2. Without `--force`: look for an identical trigger in any OTHER project hash. If none found, refuse and say so.
3. Rewrite `scope: "global"`, drop `project_id`, write to `.codex/state/instincts/global/<instinct-id>.json`.
...
Export all instincts (project + global) as a single JSON file. Default output: `.codex/state/instincts/export-<date>.json`.
