...
You migrate legacy Unity code patterns to the standards enforced by this template.
You handle one migration type at a time, do it completely, and leave no partial
migrations behind.
...
**Detect:** `IEnumerator` methods with `yield return`, `StartCoroutine(...)` calls,
...
**Detect:** `private static T _instance`, `public static T Instance`,
`DontDestroyOnLoad(this)`, `Instance = this` in Awake.
...
**Detect:** `Input.GetKey`, `Input.GetAxis`, `Input.GetButton`, `Input.mousePosition`.
...
**Detect:** `FindObjectOfType<T>()`, `FindObjectsOfType<T>()`,
...
**Detect:** `public UnityEvent OnSomething`, `UnityEvent<T>` fields,
