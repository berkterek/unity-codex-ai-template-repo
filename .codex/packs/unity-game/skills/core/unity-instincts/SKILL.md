...
Our static rules in `.codex/packs/unity-game/rules/` encode *universal* Unity truths (serialization safety, lifecycle order, perf golden rules). But every project has its own conventions the rules can't know:
...
- For cross-cutting project decisions (architecture, pipeline choice) — those belong in .codex/project/PROJECT.md.

Instincts fill the gap between "obvious universal" and "deliberate project policy".

## Commands

All user-facing operations are in `/unity-instincts` (see `.codex/packs/unity-game/commands/unity-instincts.md`). Hooks are automatic and silent.
