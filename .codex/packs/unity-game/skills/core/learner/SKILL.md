...
Append the learning to the project's `.codex/project/PROJECT.md` file under a `## Project Learnings` section. If that section does not exist, create it at the end of the file.

Each entry is appended chronologically. Never overwrite or edit previous entries — they form a timeline of discoveries.

Session data is automatically recorded to `.codex/packs/unity-game/state/learnings.jsonl` by the `auto-learn.sh` hook (strict profile). For pre-v1.3.0 projects, the file may be at `.codex/packs/unity-game/learnings.jsonl` instead.
...
- Generic Unity best practices (already covered by `.codex/packs/unity-game/rules/`)
...
- Preferences or style choices (already covered by `.codex/packs/unity-game/rules/csharp-unity.md`)
